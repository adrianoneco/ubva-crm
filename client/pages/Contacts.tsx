      ...
      <div className="min-h-screen h-screen">
        ...
        <div className="flex gap-6 items-start">
          ...
          <aside className="w-80 hidden lg:block">
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 max-h-[calc(100vh-5rem)] overflow-y-auto p-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Listas de Transmissão</h3>
                <button onClick={() => setShowNewListModal(true)} className="px-2 py-1 text-sm rounded-lg bg-primary-50 dark:bg-primary-900/30 text-primary-600">Nova</button>
              </div>

              {broadcastLists.length === 0 ? (
                <div className="text-sm text-gray-500">Nenhuma lista de transmissão</div>
              ) : (
                <div className="space-y-2">
                  {broadcastLists.map(l => (
                    <div key={l.id} className={`p-3 rounded-lg border ${selectedListId === l.id ? 'border-primary-400 bg-primary-50 dark:bg-primary-900/30' : 'border-gray-200 dark:border-gray-700' } flex items-center justify-between`}>
                      <div className="min-w-0 pr-3">
                        <div className="font-medium text-sm truncate">{l.name}</div>
                        {l.description && <div className="text-xs text-gray-500 truncate">{l.description}</div>}
                      </div>
                      <div className="flex items-center gap-2">
                        <button onClick={() => setSelectedListId(l.id)} className="text-sm text-primary-600">Abrir</button>
                        <button onClick={() => deleteBroadcastList(l.id)} className="text-sm text-red-600">Remover</button>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  <div className="sticky bottom-0 bg-gray-50 dark:bg-gray-900 p-6 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3">
                    ...
                    <button 
                      onClick={async () => {
                        ...
                        const res = await fetch('/api/contacts/import', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contacts: contactsToImport, createLists: autoCreateLists, listBaseName: autoListBaseName }) })
                        if (res.ok) {
                          const data = await res.json()
                          // refresh contacts and broadcast lists
                          await fetchContacts()
                          if (data.listsCreated && data.listsCreated.length) {
                            await fetchBroadcastLists()
                          }
                          ...
                          if (data.listsCreated && data.listsCreated.length) {
                            alert(`✅ ${data.count} contato(s) importado(s) com sucesso! ${data.listsCreated.length} lista(s) criada(s).`)
                          } else {
                            alert(`✅ ${data.count} contato(s) importado(s) com sucesso!`)
                          }
                        } else {
                          const err = await res.json()
